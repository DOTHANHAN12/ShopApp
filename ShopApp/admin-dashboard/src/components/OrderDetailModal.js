...
const modalStyles = {
    ...
    modalContent: { backgroundColor: '#FFFFFF', padding: '30px', borderRadius: '8px', width: '90%', maxWidth: '800px', maxHeight: '90vh', overflowY: 'auto' },
    ...
    h2: { fontSize: '24px', fontWeight: 'bold', margin: 0 },
    section: { marginBottom: '20px', border: '1px solid #eee', padding: '15px', borderRadius: '4px' },
    table: { width: '100%', borderCollapse: 'collapse', marginTop: '10px', fontSize: '13px' },
    th: { backgroundColor: '#f0f0f0', padding: '10px', textAlign: 'left', borderBottom: '1px solid #ccc' },
    td: { padding: '8px 10px', borderBottom: '1px solid #eee' },
    statusSelect: { padding: '5px', border: '1px solid #C40000', borderRadius: '4px', fontWeight: 'bold' },
...
const OrderDetailModal = ({ order, onClose, onSave }) => {
    ...
    // Giả định email có thể nằm ở một trường khác (ví dụ: order.email)
        ...
        <div style={modalStyles.overlay} onClick={onClose}>
            <div style={modalStyles.modalContent} onClick={(e) => e.stopPropagation()}>
                ...
                <div style={modalStyles.section}>
                    <h3 style={{ marginBottom: '10px', fontSize: '16px', fontWeight: 'bold' }}>Thông tin Khách hàng & Vận chuyển</h3>
                ...
                {/* CHI TIẾT SẢN PHẨM */}
                <div style={modalStyles.section}>
                    <h3 style={{ marginBottom: '10px', fontSize: '16px', fontWeight: 'bold' }}>Các mặt hàng ({order.items?.length || 0})</h3>
                    <table style={modalStyles.table}>
                        ...
                        <tbody>
                            {order.items?.map((item, index) => {
                                // LẤY MÀU VÀ SIZE TỪ TRƯỜNG VARIANT
                                ...
                                <tr key={index}>
                                    <td style={modalStyles.td}>
                                        {/* Hiển thị ảnh (Giả định item có trường item.productImage) */}
                ...
                {/* TỔNG KẾT THANH TOÁN */}
                <div style={modalStyles.section}>
                    <h3 style={{ marginBottom: '10px', fontSize: '16px', fontWeight: 'bold' }}>Tổng kết</h3>
                    <p><strong>Tổng phụ:</strong> {formatCurrency(order.subtotal)}</p>
                    <p><strong>Giảm giá Voucher ({order.voucherCode || 'N/A'}):</strong> - {formatCurrency(order.discountAmount)}</p>
                    <p style={{ fontSize: '18px', fontWeight: 'bold', marginTop: '10px', color: '#C40000' }}>
